...
show tables;
select * from user;
SELECT * FROM place WHERE detail_address LIKE '서울%';
SELECT 
    CAST(COALESCE(ROUND(AVG(CASE WHEN r.is_deleted = false THEN r.star ELSE NULL END)), 0) AS SIGNED) AS average_star, 
    CAST(COUNT(CASE WHEN r.is_deleted = false THEN r.review_id ELSE NULL END) AS SIGNED) AS total_review_count ,
    p.place_id, 
    p.place_name, 
    p.location, 
    p.detail_address, 
...
FROM 
    Place p 
JOIN 
    place_category pc ON p.place_ctg_id = pc.place_ctg_id 
LEFT JOIN 
    Review r ON p.place_id = r.place_id 
WHERE 
    p.detail_address LIKE '서울%' 
GROUP BY 
    p.place_id, 
    p.place_name, 
    p.location, 
...
--
INSERT INTO user (created_at, email, is_blacklist, is_deleted, login_id, nickname, phone_num, profile_img, updated_at, user_pw) VALUES
	('2024-10-15 14:23:46', '1234@tt22.com', 0, 0, 'user01', '하늘빛', '01012341234', NULL, '2024-10-15 14:23:46', '$2a$10$cZl1IvRic3jFaQcsh0uKCuS.O9ihRhHJywn6P4iHJi1UzZDsisRPq'),
	('2024-09-22 09:15:22', '2345@test.com', 0, 0, 'user02', '별빛', '01023452345', NULL, '2024-09-22 09:15:22', '$2a$10$cZl1IvRic3jFaQcsh0uKCuS.O9ihRhHJywn6P4iHJi1UzZDsisRPq');
...
(1, '롯데월드', '송파구', '서울특별시 송파구 잠실동 40', 37.5125, 127.1020, 'lotteworld.jpg', '2024-01-01', 0, 'http://www.lotteworld.com', '02-1234-5678'),
(1, '에버랜드', '경기도', '경기도 용인시 처인구 포곡읍 201-1', 37.2953, 127.1826, 'everland.jpg', '2024-01-01', 0, 'http://www.everland.com', '031-2345-6789'),
(1, '서울랜드', '경기도', '경기도 과천시 광명로 181', 37.4845, 126.9933, 'seoulland.jpg', '2024-01-01', 0, 'http://www.seoulland.co.kr', '02-3456-7890'),
(1, '키자니아', '송파구', '서울특별시 송파구 올림픽로 240', 37.5149, 127.1071, 'kidzania.jpg', '2024-01-01', 0, 'http://www.kidzania.co.kr', '02-4567-8901');
show tables;
select * from manager;
select * from review;
select * from user;
INSERT INTO user (created_at, email, is_blacklist, is_deleted, login_id, nickname, phone_num, profile_img, updated_at, user_pw) VALUES
	('2024-10-15 14:23:46', '1234@tt22.com', 0, 0, 'user01', '하늘빛', '01012341234', NULL, '2024-10-15 14:23:46', '$2a$10$cZl1IvRic3jFaQcsh0uKCuS.O9ihRhHJywn6P4iHJi1UzZDsisRPq'),
	('2024-09-22 09:15:22', '2345@test.com', 0, 0, 'user02', '별빛', '01023452345', NULL, '2024-09-22 09:15:22', '$2a$10$cZl1IvRic3jFaQcsh0uKCuS.O9ihRhHJywn6P4iHJi1UzZDsisRPq'),
    ('2024-09-22 09:15:22', '3456@test.com', 0, 0, 'user03', 'user03', '01023452343', NULL, '2024-09-22 09:15:22', '$2a$10$cZl1IvRic3jFaQcsh0uKCuS.O9ihRhHJywn6P4iHJi1UzZDsisRPq');
    


INSERT IGNORE INTO Review (place_id, user_id, star, review_content, is_deleted, created_at, updated_at)  VALUES
(54, 2, 5, '이 장소는 정말 좋았어요!', false,
    DATE_FORMAT(DATE_ADD('2023-10-01', INTERVAL FLOOR(RAND() * 31) DAY), '%Y-%m-%d %H:%i:%s'),
    DATE_FORMAT(DATE_ADD('2023-10-01', INTERVAL FLOOR(RAND() * 31) DAY), '%Y-%m-%d %H:%i:%s')),
(32,2, 4, '아주 만족스러운 경험이었습니다.', false,
    DATE_FORMAT(DATE_ADD('2023-10-01', INTERVAL FLOOR(RAND() * 31) DAY), '%Y-%m-%d %H:%i:%s'),
    DATE_FORMAT(DATE_ADD('2023-10-01', INTERVAL FLOOR(RAND() * 31) DAY), '%Y-%m-%d %H:%i:%s'));
...
select place_ctg_id, count(*) from place group by place_ctg_id;
select place_name from place where place_ctg_id=3;
desc place;


select * from likes order by likes_id desc;
select * from review r where r.place_id = 54;
