 ...
 * 튜플이라 고정된 개수가 변하면 안되지만, 변해버렸다. 
 * -> 타입스크립트의 타입 시스템이 배열 메소드와 관련된 동작을 '완전히' 제한하지 않기 때문이다. 
 * 
 * 이것 처럼 타입 시스템의 의도와 어긋나는 동작을 방지하고자 한다면, 
 * as const 를 사용하거나, readOnly 키워드를 사용하여 튜플을 불변으로 만들 수 있다. 
 */
