...
const counterSlice = createSlice({
    ...
    initialState:{number:100, isVisible:true},
    reducers:{
        plus1 : (state) => {
            state.number +=1; // 상태 직접 변경 (immer 라이브러리를 내부적으로 사용한다.)
        },
        minus1:(state) => {
            state.number -=1; 
        },
        changeVisibility1 : (state) =>{
...
export const {plus1,minus1,changeVisibility1} = counterSlice.actions;
