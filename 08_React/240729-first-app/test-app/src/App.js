...
function App() {
  ...
  return (
    ...
    <div className='App'>
      ...
      {/* JSX 문법 */}
      {/* #1. 하나로 감싸인 요소 */}
      {/* 의미없는 div 가 싫다. -> <></> Fragment 문법 */}

      {/* #2. js 문법 사용 
        - {} 로 감사면 js 표현식 사용 가능. 
        - {} 안에서 삼항 연산자 사용 가능 (if,for 사용 불가)
      */}
      <div>{name} 안녕? </div>
      <div>
        {student === 'sesac' ? <span>새싹크루</span> : <span>stranger</span>}
      </div>
      {/* #3. style 속성
         - 리액트에서 DOM 요소에 스타일 적용시 문자열 x -> 객체 사용!
         - {{스타일명 : '값'}}
         - 스타일 이름 중 하이픈(-) 포함시 camelCase로 작성해야 함.(ex. backgroundColor) 
         - font-size (x) JSX에서 - 는 연산자 (-) 이다.
      */}
      <div style={styles}>스타일 적용</div>
      <div style={{ backgroundColor:'yellow',color:'blue',fontSize : '48px' }}> 스타일 적용 2 </div>
