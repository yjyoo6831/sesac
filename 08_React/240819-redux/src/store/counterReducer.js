...
// ** toolkit 사용하지 않는 버전 (전동 Redux방식) 

// #1. 액션 타입 상수 정의  ( counter namespace 용 일뿐)
// -코드의 가독성을 높이고, 네임스페이스를 명확하게 구분 하여 이중 충돌을 방지하기 위함. 
...
const counterReducer = (state = initialState, action ) => {
    switch (action.type) {
        case "PLUS":
            return {number:state.number + 1} ; //number 값을 1증가
            
        case "MINUS":
            return {number:state.number - 1} ; //number 값을 1감소
