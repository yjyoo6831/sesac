...
//        return "_02_restapi/lmsres";
    ...
    public String dtoRes3(@RequestBody UserDTO userDTO){
        ...
        // 단!! 요청 형식이 JSON 또는 XML일때 사용됨 (하지만 지금은 일반 폼 전송이기때문에
        // 에러가 난다)

        // 참고. POST /dto/res3 요청의 경우, "일반 폼 전송"
        // - 즉, MIME Type이 application/www-x-form-urlencoded
        // -> 따라서, @RequestBody 어노테이션 사용시 오류 발생한다.

        // 올바르게 사용하기 위해
        // 1. "일반 폼 전송" 을 하고 있으니 @ModelAttribute 사용 (or 생략) ->9번 처럼
        // 2. 클라이언트 측에서 js 코드를 사용해 폼 데이터를 json으로 변환하여 전송 -> 동적 폼 전송 구현
        System.out.println("[POST] userDTO (name) = "+ userDTO.getName());
        System.out.println("[POST] userDTO (age) = "+ userDTO.getAge());
    ...
    public String voRes1(@ModelAttribute UserVO userVO){
        // /vo/res1?name=12&age=1
        // @ModelAttribute UserVO userVO
        // - 요청 파라미터를 UserVO객체에 바인딩

        // 참고. 브라우저에서 응답이 "null 0"으로 도착하는 이유는 ?
        // - @ModelAttribute 어노테이션은 setter 메서드를 통해 객체에 값을 주입
        // - 하지만, UserVO 에는 setter가 없으므로 폼에서 전송된 데이터가 주입되지 않음.
        // => 따라서, name/age 필드는 초기화되지 않은 상태 null과 0으로 남게 됨.
        System.out.println("[GET] userDTO (name) = "+ userVO.getName());
        System.out.println("[GET] userDTO (age) = "+ userVO.getAge());
    ...
    @PostMapping("/vo/res2")
    @ResponseBody
    public String voRes2(@ModelAttribute UserVO userVO){
        // @ModelAttribute 생략가능
        // -setter 를 이요해 객체에 값을 주입
        // -> VO 객체는 setter를 가지지 않으므로 데이터 바인딩이 제대로 이뤄지지 않음
