...
public class TodoController {
    ...
    public ResponseEntity<?> createTodo(@AuthenticationPrincipal String userId, @RequestBody TodoDTO dto){
        ...
        // 우리 코드) jwtAuthenticationFilter 클래스에서 userId를 바탕으로 인증 객체 생성했음
        try {
            // 임시 유저 하드코딩
...
//            entity.setUserId(temporaryUserId);
            ...
            // 4) 서비스 계층 이용해 todo 엔티티 생성
            List<TodoEntity> entities = service.create(entity);
            System.out.println("entities > " +  entities);

            // 5) 리턴된 엔티티 리스트(추가된 행 하나)를 TodoDTO 로 변환
            List<TodoDTO> dtos = new ArrayList<>();
            for(TodoEntity tEntitiy: entities){
                TodoDTO tDTO = new TodoDTO(tEntitiy);
                dtos.add(tDTO);
            }
            ...
            // 7) ResponseDTO를 클라이언트 응답
            return ResponseEntity.ok().body(response);

        }catch (Exception e){
            // 8) 예외가 발생한 경우, ResponseDTO의 필드 대신, error 필드에 메세지를 넣어서 리턴
