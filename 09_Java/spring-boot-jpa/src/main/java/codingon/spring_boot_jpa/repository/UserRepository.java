...
//    boolean existsByUsername(String username);
    ...
    // - JpaRepository 인터페이스에 내장된 메소드 만으로는 해결이 안되는 경우, (raw query 작성 필요)
    // - 단, JPA는 테이블명이 아니라 객체 위주로 돌아가니 객체(Entity) 이름을 사용해야 함.
    // - nativeQuery 옵션
    // : SQL 문을 사용하는 거라 DBMS 에 종속적이게 됨. (ORM 특징을 활용하기 어려움)
    ...
    List<User> findByUsername(@Param("username") String username); //  select * from user where username  = ?
